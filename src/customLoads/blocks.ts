...
import { onceWithCleanup } from "./promise_utils";
import BrokenImport, { Block, Block as IBlock } from "prismarine-block";
import BrokenImportTwo from "prismarine-chunk";
import { RaycastIterator } from "../classes/iterators/raycast/Raycast";
const { OctahedronIterator } = require("prismarine-world").iterators;

class Painting {
    constructor(public id: number, public position: string, public name: string, public direction: Vec3) {}
}

const paintingFaceToVec = [new Vec3(0, 0, -1), new Vec3(-1, 0, 0), new Vec3(0, 0, 1), new Vec3(1, 0, 0)];
...
export function inject(bot: Bot, { version, storageBuilder }: { version: number; storageBuilder: any }) {
    const Block: typeof IBlock = (BrokenImport as any)(version);
    const Chunk: ReturnType<ReturnType<typeof BrokenImportTwo>> = (BrokenImportTwo as any)(version);
    const World = require("prismarine-world")(version);

    const paintingsByPos: { [pos: string]: Painting } = {};
    const paintingsById: { [id: number]: Painting } = {};
            ...
            return new Promise((resolve) => {
                function waitForLoadEvents(columnCorner: Vec3) {
                    ...
                    if (chunkPosToCheck.size === 0) {
                        ...
                        resolve(undefined); // used to be empty.
                    }
                }
    ...
    function getMatchingFunction(matching: FindBlockOptions["matching"]) {
        ...
        // we have asserted matching is number[]
        function isMatchingType(block: Block) {
            return block === null ? false : (matching as number[]).indexOf(block.type) >= 0;
        }
    }

    function isBlockInSection(section: { palette: any }, matcher: (block: Block) => boolean) {
    ...
    function getFullMatchingFunction(matcher: (block: Block) => boolean, useExtraInfo: boolean | ((block: Block) => boolean)) {
        ...
        function nonFullSearchMatcher(point: Vec3) {
            ...
            return matcher(block) && (useExtraInfo as (block: Block) => boolean)(block);
        }

        // asserted that useExtraInfo is boolean.
    ...
    function findBlock(options: FindBlockOptions) {
    ...
    bot._client.on("spawn_entity_painting", (packet) => {
        ...
        const painting = new Painting(packet.entityId, pos, packet.title, paintingFaceToVec[packet.direction]);
    ...
    async function switchWorld() {
        if (bot.world) {
            ...
            bot.world = new World(
                null,
                storageBuilder ? storageBuilder({ version: bot.version, worldName: dimensionToFolderName(dimension) }) : null
            ).sync;
            startListenerProxy();
        }
    }
