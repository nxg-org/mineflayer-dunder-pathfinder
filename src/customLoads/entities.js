...
const conv = require("@nxg-org/mineflayer-util-plugin").MathUtils;
...
function inject(bot, { version }) {
    ...
    bot._client.on("entity_equipment", (packet) => {
        ...
        if (packet.equipments !== undefined) {
            packet.equipments.forEach((equipment) =>
                entity.setEquipment(equipment.slot, equipment.item ? Item.fromNotch(equipment.item) : null)
            );
        } else {
    ...
    bot._client.on("entity_metadata", (packet) => {
        ...
        const poseField = packet.metadata.find((p) => p.key === 6);
        if (poseField !== undefined) {
            if (entity.pose !== poseField) {
                entity.pose = poseField; // new field
                bot.emit("entityPoseChange", entity, poseField);
            }
        }
    });
